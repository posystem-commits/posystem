...
// renewal popup itself (see the tenantStatus effect in src/pos.jsx) instead of that living in a
// server-dispatched email. Deliberately NOT gated by requireActiveTenant like the storage routes
// — a paused or expired tenant still needs to be able to ask "what's my status?" (that's the
// whole point), it just gets told the truth rather than being locked out of finding out.
export async function GET(req, { params }) {
  const { data: tenant, error } = await supabaseAdmin()
    .from("tenants")
    .select("status, paid_until, restaurant_name")
